...
public class BonusDAOImpl extends AbstractDAO implements BonusDAO{
	...
	public List<Integer> getDepartments(){
		...
		try (Connection conn = ConnectionFactory.getConnection();
				...
				PreparedStatement ps = conn.prepareStatement("select deptNo from tbl_bonus")) {
				rs=ps.executeQuery();
				while(rs.next()){
	...
	public void loadBonus(List<Bonus> bonus) {
		...
		try (Connection conn = ConnectionFactory.getConnection();
				Statement st=conn.createStatement();
				PreparedStatement ps = conn.prepareStatement("insert into tbl_bonus values(?,?,?)")) {
			logger.info("batch processing file data");
			conn.setAutoCommit(false);
			st.execute("truncate tbl_bonus");
			int count = 1;
	...
	public List<Bonus> getAllBonus() {
		...
		try (Connection conn = ConnectionFactory.getConnection();
				Statement st=conn.createStatement();
				PreparedStatement ps = conn.prepareStatement("select deptNo,amount,remainingAmount from tbl_bonus")) {
	...
	public void updateBonus(int deptNo,float amount) {
		...
		try (Connection conn = ConnectionFactory.getConnection();
				Statement st=conn.createStatement();
				PreparedStatement ps = conn.prepareStatement("update tbl_bonus set remainingAmount=? where deptNo=?")) {
